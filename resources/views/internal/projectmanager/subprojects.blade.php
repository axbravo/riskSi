...
@section('content')
        ...
        <table class="table table-bordered table-striped">
            ...
            <tr>
                ...
                <td class="button-center"><a class="btn btn-info" href={{route('projects.edit', $subactivity->id)}} title="Editar" ><i class="glyphicon glyphicon-pencil"></i></a>
                </td> 
                ...
                <div class="modal-dialog">
                  <div class="modal-content">
                    ...
                    <div class="modal-footer">
                      <form method="post" action={{route('projects.delete', $subactivity->id)}}>
                        {!! csrf_field() !!}
        ...
        <div class="row">
          <div class="col-md-12 text-right">
            <a href="{{route('projectmanager.projects.index')}}" class="btn btn-info">Regresar</a>
          </div>
        </div>
 <html>
  <head>
      ...
      function drawChart() {
        ...
        for (key in info){
           ...
           //Poner actividad inicial
            if(info[key].name!='Verificación de huellas digitales')
            data.addRows([[info[key].name, info[key].name,new Date(initialYear,initialMonth, initialDay),new Date(finalYear,finalMonth,finalDay),info[key].Duration,0,info[d_id_activity].name]]);
            else
             data.addRows([[info[key].name, info[key].name,new Date(initialYear,initialMonth, initialDay),new Date(finalYear,finalMonth,finalDay),info[key].Duration,0,null]]); 
          }
         var options = {
          height: 275,
          gantt: {
